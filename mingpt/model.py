...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, hidden=False):
        ...
        if hidden:
            hid_act = []
            hid_act.append(token_embeddings)
        ...
        loss = None
        if targets is not None: # TODO: do we need to modify loss here?
...
class GPTforFFT(GPT): # GPT for fine-tuning with frozen layers

    def __init__(self, config, layers_to_freeze = []):
        super(GPTforFFT, self).__init__(config)

        for i in layers_to_freeze:
            for param in self.blocks[i].parameters():
                param.requires_grad = False

        self.layers_to_freeze = ['blocks.' + str(i) for i in layers_to_freeze]
    ...
    def configure_optimizers(self, train_config):
        ...
        for mn, m in self.named_modules():
            ...
            for pn, p in m.named_parameters():
                ...
                if fpn[:8] in self.layers_to_freeze or fpn == 'tok_emb':
                    print('Freezing: ', fpn) # debug
                    continue
        ...
        # create the pytorch optimizer object
        optim_groups = [
            {"params": [param_dict[pn] for pn in sorted(list(decay))], "weight_decay": 0.1}, # train_config.weight_decay
            {"params": [param_dict[pn] for pn in sorted(list(no_decay))], "weight_decay": 0.0},
        ]
        optimizer = torch.optim.AdamW(optim_groups, lr=train_config.learning_rate, betas=train_config.betas)
        return optimizer
